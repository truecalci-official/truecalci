...
// -----------------------------------------------------------------------------
// MCP Resources, Resource Templates & Prompt Workflows
// -----------------------------------------------------------------------------
const MCP_RESOURCES = [
  {
...
function readResourceContent(uri) {
  ...
  if (formulaMatch) {
    ...
    const tool = MCP_TOOL_DEFINITIONS.find(t => t.name === calcId);
    if (tool) {
      return {
        contents: [
          {
    ...
    if (isMcpRoute) {
      if (request.method === "GET") {
        ...
        return new Response(JSON.stringify({
          ...
          supportedMethods: ["initialize", "tools/list", "tools/call", "resources/list", "resources/read", "resources/templates/list", "prompts/list", "prompts/get", "ping"]
        }, null, 2), {
          status: 200,
          headers: {
            "Content-Type": "application/json; charset=utf-8",
      ...
      if (request.method === "POST") {
        try {
          ...
          if (method === "resources/read") {
            ...
            try {
              const resContent = readResourceContent(uri);
              return new Response(JSON.stringify({
                jsonrpc: "2.0",
                id,
                result: resContent
          ...
          if (method === "prompts/get") {
            ...
            try {
              const promptData = getPromptResponse(promptName, promptArgs);
              return new Response(JSON.stringify({
                jsonrpc: "2.0",
                id,
                result: promptData
